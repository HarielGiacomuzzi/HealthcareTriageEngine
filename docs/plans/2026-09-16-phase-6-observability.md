...
*(Filled in as the phase is implemented — one line per deviation, each with the phase that closes it. Known candidates going in: the `observability` compose profile is not built (user's decision, not scheduled); `ecet_db_pool_in_use` is an addition to the spec's metric table, not one of its rows; the api and the worker each expose their own registry, so a scraper needs both targets; two concurrent resolves can both POST before one loses at `task.resolve`; a crash between the read and the write unit of work in UC-06 leaves the claim `QUEUED` for redelivery.)*
