...
13. **The webhook POST behind resolve and retry runs inside the HTTP request and the open unit of work.** This is the api-side twin of Phase 4 deviation 17, and is recorded, not restructured, for the same reason. Unlike the worker, the api path has no prefetch-equivalent bound on concurrency, and `/readyz`'s database probe draws on the same pool, so a resolve or retry that blocks on a black-holing webhook can exhaust the pool and take the api container unhealthy faster than the worker's twin. Closed by Phase 6, which must move the POST out of the request rather than only add a metric. (The in-request client itself now retries once, not `ECET_WEBHOOK_MAX_ATTEMPTS` times, since `retry-notify` is already the retry path — see the final-review fix report.)
