...
4. **Two transition edges are added beyond the state list**: `NOTIFY_FAILED → APPROVED_AUTO | REVIEW_RESOLVED` (required by `POST /v1/claims/{id}/retry-notify` in the [api spec](../../specs/04-interfaces/api.md), Phase 5) and `EVALUATION_FAILED → REVIEW_RESOLVED` ([UC-06](../../specs/02-use-cases/UC-06-evaluate-claim.md) opens a review task with reason `EVALUATION_FAILED`, and [UC-09c](../../specs/02-use-cases/UC-09-human-review.md) resolves it). Without them those two specified flows have no legal end state.
