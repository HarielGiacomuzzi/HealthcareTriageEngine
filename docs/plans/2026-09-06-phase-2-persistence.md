...
7. **Optimistic concurrency is tracked inside `PostgresClaimRepository`** (a `dict[UUID, datetime]` of the last `updated_at` the repository saw) rather than through SQLAlchemy's `version_id_col`. Closes Phase 1 carry-over #1: `Claim` has no version field, `transition()` overwrites `updated_at` in place, and the schema has no version column. A claim the repository never loaded cannot be saved. Three limits the whole-branch review named, worth stating next to the design: **(a) ABA.** The guard compares a timestamp *value*, not a monotonic counter, so a competing writer that happens to land the same `updated_at` — a coarse or fixed injected `Clock`, a replay — is invisible to it. **(b) Scope.** It protects `claims` only; `review_tasks` is guarded by the status check in deviation 8 instead of a timestamp, since a resolution has no meaningful "same instant, different writer" case to miss. **(c) Per-instance baseline.** A caller that saves without reading first through *this* repository instance gets a hard `ConcurrentModification`, not a silent overwrite — deliberate, but it is a runtime surprise the caller has to know to expect (mirrored in `FakeClaimRepository` so a use case tested against the fake sees it too).
...
18. **The mappers are named `*_from_row()` / `*_to_row_values()`, not `to_domain()` / `from_domain()`** as the [postgres spec](../../specs/03-infrastructure/postgres.md#mappers) sketches. The dict form (`*_to_row_values()`) feeds `insert()` and `update()` directly; an object-returning `from_domain()` could not.
